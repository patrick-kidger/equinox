...
def main(dataset_size=10000, batch_size=256, learning_rate=3e-3, steps=1000, width_size=8, depth=1, seed=5678):
    ...
    for step, (x, y) in zip(range(steps), data):
        ...
        print(step, value)
    print(f"Weights of first layer at initialisation: {jax.tree_flatten(original_model.layers[0])}")
    print(f"Weights of first layer at end of training: {jax.tree_flatten(model.layers[0])}")
    print(f"Weights of last layer at initialisation: {jax.tree_flatten(original_model.layers[-1])}")
    print(f"Weights of last layer at end of training: {jax.tree_flatten(model.layers[-1])}")
