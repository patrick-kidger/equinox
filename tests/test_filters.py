...
def test_is_inexact_array(getkey):
    objs = [1, 2., [2.], True, object(), jnp.array([1]), jnp.array(1.), eqx.nn.Linear(1, 1, key=getkey())]
...
def test_split_and_merge(getkey):
    ...
    for pytree in ([1, 2, [3, "hi", {"a": jnp.array(1), "b": 4,
                                     "c": eqx.nn.MLP(2, 2, 2, 2, key=getkey())}]], [1, 1, 1, 1,
                                                                                    "hi"]):  # has repeated elements
...
def test_splittree_and_merge(getkey):
    ...
    filter_tree = [True, False, [False, False, {"a": True, "b": False, "c": linear_tree}]]
    for i, pytree in enumerate(([1, 2, [3, True, {"a": jnp.array(1), "b": 4, "c": linear}]],
                                [1, 1, [1, 1, {"a": 1, "b": 1, "c": linear}]])):  # has repeated elements
        keep_args, notkeep_args, which, treedef = eqx.filters.split_tree(pytree, filter_tree)
        if i == 0:
