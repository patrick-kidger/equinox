...
def test_dot_product_attention_weights(getkey):
    ...
    key_ = jrandom.uniform(getkey(), (23, 4, 3))

    attn_weights = eqx.nn.attention.dot_product_attention_weights(
        query=query,
        key_=key_,
    )
...
def test_dot_product_attention(getkey):
    query = jrandom.uniform(getkey(), (19, 4, 3))
    key_ = jrandom.uniform(getkey(), (23, 4, 3))
    value = jrandom.uniform(getkey(), (23, 4, 7))

    attn = eqx.nn.attention.dot_product_attention(
        query=query,
        key_=key_,
        value=value,
    )
